...
def write_csv(filename, coll):
    ...
    with open(filename, "w") as file_out:
      ...
      for card in g_user.find_cards(collection=coll):
        ...
        row.append(card.get_full_name('original_owner'))
        row.append(card.last_modified_date)
        row.append(card.get_full_name('last_modified_by') if card.last_modified_by else None)
        row.append(card.last_verified_date)
        row.append(card.get_full_name('last_verified_by') if card.last_verified_by else None)
